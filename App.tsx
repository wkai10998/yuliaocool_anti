...
import { AppMode, CorpusItem, ContextScenario } from './types';
import { BookOpen, Mic2, Library, Zap, Clock, TrendingUp } from 'lucide-react';
import { generateContextScenario } from './services/zhipuService';
...
export default function App() {
  ...
  // === 主页预取语境实战 ===
  const REVIEW_CACHE_KEY = 'yuliaocool_review_active_session';
  const REVIEW_CACHE_EXPIRY_MS = 3600 * 1000; // 1 hour
  const isPrefetchingReviewRef = useRef(false);

  // 智能选词（与 ReviewMode 保持一致）
  const selectSmartPhrases = (corpusList: CorpusItem[], count: number): string[] => {
    const now = Date.now();
  ...
  // 主页预取语境实战第一关
  useEffect(() => {
    if (mode !== AppMode.DASHBOARD || corpus.length === 0) return;
    if (isPrefetchingReviewRef.current) return;
    ...
    if (cached) {
      try {
        ...
        if (Date.now() - timestamp < REVIEW_CACHE_EXPIRY_MS) {
          console.log('[App] 语境实战缓存有效，跳过预取');
          return;
        }
      } catch (e) {
    ...
    const timer = setTimeout(async () => {
      if (isPrefetchingReviewRef.current) return;
      isPrefetchingReviewRef.current = true;

      console.log('[App] 开始主页预取语境实战...');
      try {
        const items = selectSmartPhrases(corpus, 5);
        const topic = '日常对话';
        const result = await generateContextScenario(items, topic);
        ...
        console.log('[App] 语境实战预取完成');
      } catch (e) {
        console.warn('[App] 语境实战预取失败:', e);
      } finally {
        isPrefetchingReviewRef.current = false;
      }
    }, 2000);
